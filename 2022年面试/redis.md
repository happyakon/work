...
>
> select模式一共只能监视1024个文件描述符，使用轮训的机制查看文件描述符的状态，epoll比较优秀的是没有1024的限制，并且是事件驱动，当某个FD准备好了之后会触发事件，epoll返回用户的也是准备好的FD，不需要再去遍历，而且数据结构上用了红黑树和链表。这部分是放在内核和用户共用的部分，不用再从用户态到内核态的文件复制。
...
> 4. 二进制安全
>
>    C是扫描一个特定的字符串，而Redis有len记录，这样就能保证redis可以存二进制数据，不用担心特殊字符带来的影响。
...
> 和hashMap类似，先计算hash值，如果冲突就采用头插法插入链表，因为是单线程，所以不会有HashMap的问题
